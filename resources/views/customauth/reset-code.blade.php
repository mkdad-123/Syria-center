...
<head>
    ...
    {{-- مهم لاستخدام fetch مع POST --}}
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>{{ __('auth.reset_request_title') }} - {{ __('main.site_name') }}</title>
...
<body>
    @php
        ...
        // مولد رابط تبديل اللغة مع الحفاظ على نفس الصفحة والـ query string
        $swapLocaleUrl = function (string $lang) {
            $segments = request()->segments(); // مثل: ['ar','password','request']
    ...
    <!-- قسم طلب رمز إعادة التعيين -->
    <div class="reset-page">
        <div class="reset-container">
            <h2 class="reset-title">{{ __('auth.reset_request_title') }}</h2>
            ...
            <form id="resetRequestForm" method="POST"
                action="{{ route('password.reset-code', ['locale' => $locale]) }}">
                @csrf
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-block" id="submitBtn">{{ __('auth.send_code_btn') }}</button>
                </div>
            </form>

            <div class="reset-footer">
                <a href="{{ route('login', ['locale' => $locale]) }}">{{ __('auth.back_to_login') }}</a>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                btn.textContent = '{{ __('auth.sending') }}';
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            // انتقال إلى صفحة إدخال الرمز مع تمرير البريد
                            window.location.href = `/${lang}/verify-code?email=` + encodeURIComponent(
                                data.email);
                        } else {
                            // عرض أخطاء التحقق (422) أو رسالة عامة
                            if (data.errors && data.errors.email) {
                                emailError.textContent = data.errors.email[0];
                                emailError.classList.remove('d-none');
                            } else {
                                errorBox.textContent = data.message ||
                                    '{{ __('auth.error_sending_code') }}';
                                errorBox.classList.remove('d-none');
                            }
                        }
                    })
                    .catch(() => {
                        errorBox.textContent = '{{ __('auth.error_sending_code') }}';
                        errorBox.classList.remove('d-none');
                    })
                    .finally(() => {
                        btn.disabled = false;
